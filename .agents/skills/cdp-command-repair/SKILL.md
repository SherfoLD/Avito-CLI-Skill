---
name: cdp-command-repair
description: Use when a command that used to work now fails — a typed error at runtime, a red offline suite, or a verify fixture that stopped matching. Guides diagnosis, a bounded repair, and writing the result into memory. For writing a new command, use cdp-command-author instead.
...
| `CommandExecutionError` about shape | drift: Avito changed the payload | the carrier, compared against a stored evidence sample |
...
against the stored sample in `fixtures/` for that command.

The comparison is the point. `fixtures/` exists so that "Avito changed" is a
...
1. **Avito changed shape.** The decoder needs to learn the new shape. The
   ...
   that shows the new shape goes into `fixtures/`.
...
- **Never relax a verify fixture to silence a failure.** A failing `patterns`,
  `notEmpty`, `mustNotContain` or `mustBeTruthy` rule means the output is broken.
  Tighten the command. The one legitimate reason to edit a fixture during a
...
- The evidence, in `fixtures/`: the anonymised sample you compared against, so
