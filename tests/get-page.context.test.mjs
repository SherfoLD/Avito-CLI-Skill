...
// from src/decoders/get-page.mjs against a synthetic Avito SSR carrier and a stubbed fetch.
...
import { paginate } from '../src/decoders/get-page.mjs';
...
check('one document decodes page-2 rows with the visible price, metro line and card text', async () => {
  ...
  // The SSR carrier leaves the flat description empty, which is what made this column
  // always null before the visible decoder was carried over from search.
...
check('the largest photo variant wins and an unknown size set fails closed', async () => {
  ...
  const renamed = makeFetch([documentRoute(pageState({
  ...
  const unusable = makeFetch([documentRoute(pageState({
...
// Reservation is read only from the flat boolean Avito puts on every catalog card; an
// absent key stays null so that only --remove-reserved can turn it into a stop (F-048).
