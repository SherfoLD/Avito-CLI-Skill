...
- **Искать и листать.** Запрос, локация (город, метро, район, координаты с радиусом),
  вторая и следующие страницы выдачи.
- **Сужать выдачу.** Читать и применять все фильтры категории.
...
- **Отдавать данные как данные.** Каждая команда печатает один JSON-объект: то,
  что верно для всего ответа, лежит на верхнем уровне, а различающееся — внутри
  списка. У каждой команды строгая схема — ответ либо проходит её целиком, либо
  команда падает с типизированной ошибкой. Правдоподобных, но неверных данных не
  бывает.
...
search "ddr5 32gb" --location-id …  → searchUrl + items[]
